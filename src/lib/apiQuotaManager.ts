...
export class APIQuotaManager {
  ...
  trackAPICall(): void {
    ...
    console.log(`📊 API call tracked. Daily: ${this.dailyUsage}/${this.dailyLimit}, Per-second: ${this.currentSecondUsage}/${this.perSecondLimit}`);
  }
  ...
  estimateQuotaForCity(
    ...
    console.log(`📊 Quota estimate for ${hexagonCount} hexagons:`);
  ...
  optimizeProcessingStrategy(
    ...
    console.log(`🔧 Processing optimization for ${totalHexagons} hexagons:`);
    console.log(`🔧 Max hexagons per day: ${maxHexagonsPerDay}`);
    console.log(`🔧 Recommended batch size: ${maxBatchSize}`);
    console.log(`🔧 Estimated processing time: ${(estimatedProcessingTime / 1000 / 60).toFixed(1)} minutes`);
    console.log(`🔧 Cost efficiency: ${(costEfficiency * 100).toFixed(1)}%`);
  ...
  canProcessRequest(estimatedCalls: number): boolean {
    ...
    if (!canProcess) {
      console.warn(`⚠️ Cannot process request: ${estimatedCalls} calls needed, ${remainingQuota} remaining`);
    }
